...
export class ApiFileManager extends AppManager {
  ...
  public downloadUnitBuffer(options: DownloadOptions): CancellablePromise<Uint8Array> {
    const log = this.log.bindPrefix('download');
    ...
    const fileName = getDownloadFileNameFromOptions(options);
    const cacheFileName = downloadId ? getDownloadFileNameFromOptions({...copy(options), downloadId: undefined}) : fileName;
    let deferred = deferredPromise<Uint8Array>();
    ...
    const isLocalWebFile = isWebFile && this.isLocalWebFile((location as InputWebFileLocation.inputWebFileLocation).url);
    const id = this.tempId++;
    const limitPart = isLocalWebFile ? size : options.limitPart || this.getLimitPart(size, false);

    const possibleSize = size || limitPart;
    let buffer = new Uint8Array(possibleSize);
    let done = 0;
    let cancelled = false;

    const checkCancel = () => {
      if(cancelled || deferred.isRejected) {
    ...
    function startDownload() {
      const superpuper = async() => {
        ...
        const writeDeferred = deferredPromise<void>();
        const currentWritePromise = writePromise;
        ...
        try {
          ...
          const byteLength = bytes.byteLength;
          log('requestPart result', fileName, result);
          const isFinal = (currentOffset + limitPart) >= size || !byteLength;
          ...
          if(byteLength) {
            ...
            buffer.set(new Uint8Array(bytes), currentOffset);
          }

          if(isFinal) {
            if(!size || done < size) {
              buffer = buffer.slice(0, done);
            }

            if(process) {
              buffer = await process(buffer, cacheFileName);
            }
      ...
      for(let i = 0, length = clamp(size / limitPart, 1, Infinity); i < length; ++i) {
        superpuper();
      }
    }
